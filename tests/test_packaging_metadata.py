...
def test_pyproject_declares_shared_library_versions() -> None:
    ...
    assert "daylily-tapdb @ git+https://github.com/Daylily-Informatics/daylily-tapdb.git@9.0.2" in dependencies
    assert "psycopg2-binary>=2.9.9" in dependencies
    assert tapdb_dependency == _tapdb_dependency_spec()
    assert tapdb_dependency == "daylily-tapdb @ git+https://github.com/Daylily-Informatics/daylily-tapdb.git@9.0.2"
