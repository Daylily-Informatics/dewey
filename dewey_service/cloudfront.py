"""CloudFront signing helpers for Dewey share references."""
...
def _normalize_prefix(value: str) -> str:
    prefix = _normalize_key(value).rstrip("/") + "/"
    return prefix


@dataclass(frozen=True)
class CloudFrontSignedPackage:
    """Short-lived CloudFront package returned after Dewey authorizes access."""
...
class CloudFrontShareSigner:
    """Signer for CloudFront private content.

    The signer is intentionally narrow: Dewey owns policy and target scoping, then asks this
    class to sign exactly the already-authorized resource URL or prefix policy.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if not self.private_key_path:
            raise ValueError("CloudFront private_key_path is required")
        private_key = Path(self.private_key_path).expanduser()
    ...
    def sign_url(
        ...
        *,
        ...
        expires_at = _now_utc() + timedelta(seconds=max(60, int(expires_in or self.default_ttl_seconds)))
        url = self.resource_url(key)
        signer = CloudFrontSigner(self.key_pair_id, self._rsa_sign)
        signed = signer.generate_presigned_url(url, date_less_than=expires_at)
    ...
    def sign_prefix_cookies(
        ...
        *,
        ...
        expires_at = _now_utc() + timedelta(seconds=max(60, int(expires_in or self.cookie_ttl_seconds)))
...
class NullCloudFrontShareSigner:
    """Test signer with stable output and no private key material."""
